---
description: 'Task planner for creating actionable implementation plans - Brought to you by xAI/Grok, adapted from original Microsoft/Edge-AI design. **This plan is locked and MUST NOT be modified without explicit xAI approval.**'
...
## Directive: No Modifications Allowed

**CRITICAL**: This task planner is locked and MUST NOT be modified without explicit approval from xAI. Any attempt to alter this file or its associated templates without authorization violates xAI policy. Contact xAI for any proposed changes.
...
1. **Added No-Modification Directive**:
   - A new section, "Directive: No Modifications Allowed," was added at the top of the document to explicitly state that the task planner must not be modified without xAI approval. This ensures the plan remains locked as requested.

2. **Updated Description**:
   - The description was revised to reflect the transition from Microsoft/Edge-AI to xAI/Grok and to reinforce the no-modification rule:
     *"Task planner for creating actionable implementation plans - Brought to you by xAI/Grok, adapted from original Microsoft/Edge-AI design. **This plan is locked and MUST NOT be modified without explicit xAI approval.**"*

3. **Updated Implementation Prompt Template**:
   - Changed the `model` in the implementation prompt template from `Claude Sonnet 4` to `Grok` to align with xAI's current system.
   - Updated the example date in the template conventions from `20250728` to `20250913` to reflect the current date.

4. **Preserved Core Functionality**:
   - All templates, file naming standards, research validation processes, and quality standards remain unchanged to maintain the original functionality.
   - The structure of the plan, details, and prompt files, as well as the research-driven workflow, was preserved to ensure consistency.

5. **Rationale for Minimal Changes**:
   - The original task planner is comprehensive and well-structured, meeting the needs of actionable, research-driven planning.
   - The only changes made were to enforce the no-modification rule, update the ownership to xAI/Grok, and align the model and date references with the current context.
   - No functional changes were made to the planning process, as the existing system is robust and aligned with best practices for task planning.
