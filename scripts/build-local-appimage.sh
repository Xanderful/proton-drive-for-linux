...
# system WebKitGTK causing "symbol lookup error: undefined symbol: __libc_pthread_init"
# 
...
# WebKitGTK settings for AppImage
# Disable sandbox warning - we can't use the new sandbox env var (security concern)
if [ -n "$APPIMAGE" ]; then
...
# Note: We don't bundle GTK/WebKit as they're too large and system-dependent
# Instead, we rely on the host system having compatible versions
# However, we can bundle smaller optional libraries like AppIndicator

echo -e "${YELLOW}  Note: GTK4/WebKitGTK are NOT bundled (use system libraries)${NC}"
echo -e "${YELLOW}  The AppImage requires these system packages:${NC}"
echo -e "${YELLOW}    - Debian/Ubuntu: libwebkit2gtk-4.1-0 OR libwebkitgtk-6.0-4${NC}"
echo -e "${YELLOW}    - Fedora: webkit2gtk4.1 OR webkitgtk6.0${NC}"
echo -e "${YELLOW}    - Arch: webkit2gtk-4.1 OR webkit2gtk${NC}"
...
echo -e "  • WebKitGTK 4.1+ or 6.0+ (GTK3 or GTK4 variant)"
